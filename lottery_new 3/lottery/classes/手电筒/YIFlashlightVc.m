...
- (void)loadWhyUseMeView {
	...
	lbl.text = @"Why did you choose me? \r\t 1, support 3D-Touch \r\t\t\t\t flashlight. If you are a iPhone6s series mobile phone, lucky. You can press App on the main screen icon directly using a flashlight. \r\t 2, two models of the flashlight can be switched freely. \r\t\t\t\t screen light more power and flash brighter, convenient switch to their love way oh.";
}
...
- (void)loadOnOffView {
	...
	NSMutableAttributedString *str = [[NSMutableAttributedString alloc] initWithString:@"open the \r flashlight"];
...
- (void)switchBtnAction:(UIButton *)btn {
	...
	if (btn.selected) {
		NSMutableAttributedString *str = [[NSMutableAttributedString alloc] initWithString:@"turn off the \r flashlight"];
//        [str addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:30] range:NSMakeRange(0, 2)];
//        [str addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:15.0] range:NSMakeRange(3, 3)];
		titleLbl.attributedText = str;
	} else {
...
- (void)loadFlashlightTypeView {
	...
	mySwitch3.onLabel.text = @"screen";
	mySwitch3.onLabel.textColor = kAppColorWhite;
	mySwitch3.offLabel.text = @"flash";
