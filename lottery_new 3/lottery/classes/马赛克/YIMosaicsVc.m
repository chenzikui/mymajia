...
- (void)loadToolView {
	...
	[pickerBtn setTitle:@"Select" forState:UIControlStateNormal];
...
- (void)loadToolView2 {
	...
	[paintBtn setTitle:@"Apply" forState:UIControlStateNormal];
...
- (IBAction)saveImageBtnAction:(id)sender {
	...
	hud.labelText = [NSString stringWithFormat:@"Save to the album: %@", [YICommonUtil appName]];
		...
		[assetsLibrary saveImage:[mosaicsView savedImage]
			 ...
			 withCompletionBlock:^(NSError *error) {
				 if (error) {
					 ...
					 hud.labelText = @"Save failure!";
					 hud.detailsLabelText = [NSString stringWithFormat:@"Please operate: [Setting]>[Privacy]>[Photo]>[%@]", [YICommonUtil appName]];
